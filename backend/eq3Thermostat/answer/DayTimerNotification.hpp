#ifndef THERMONATOR_EQ3THERMOSTAT_ANSWER_DAYTIMERNOTIFICATION_HPP
#define THERMONATOR_EQ3THERMOSTAT_ANSWER_DAYTIMERNOTIFICATION_HPP
...
class DayTimerNotification : public QObject {
    Q_OBJECT
public:
    explicit DayTimerNotification(QObject *parent = nullptr);
...
signals:
    void answerDecoded(types::DayOfWeek dayOfWeek,
                       const types::DayTimer &dayTimer);

    void answerNotDecodedable();

private:
    static bool answerIsValid(const QByteArray &answer);

    static std::optional<types::DayOfWeek>
    optDecodeDayOfWeek(char dayOfWeekEncoded);

    static std::optional<types::DayTimer>
    optDecodeDayTimer(const QByteArray &answer);

    static double decodeTemperature(char temperatureEncoded);
    static types::Time decodeTime(char timeEncoded);
};
